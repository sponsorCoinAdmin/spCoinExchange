...
let provider
const chainId = parseInt(process.env.GOERLI_CHAIN_ID)
let router
...
class AlphaRouterService {
  ...
  constructor() {
    this.ethers = ethers
    provider = new this.ethers.providers.JsonRpcProvider(GOERLI_INFURA_TEST_URL) 
    UTS = new UniTokenServices(ethers, chainId, provider)
    router = new AlphaRouter({ chainId: chainId, provider: provider})
    console.log( "DEBUG MODE OFF: AlphaRouterServiceDebug");
  }
  ...
  exeRouteTransaction = async( _recipientAddress, _walletPvtKey, _tokenAddrIn, _route, _gasLimit) => {
    ...
    const contractIn = new this.ethers.Contract(_tokenAddrIn, ERC20ABI, provider)
  ...
  exeExactInputTransaction = async(
    ...
    _gasLimit) => {
      ...
      let tokenInContract  = new this.ethers.Contract(_tokenInAddr, ERC20ABI, provider)
