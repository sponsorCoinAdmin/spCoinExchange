...
class AlphaRouterService {
  constructor(erc20Services2) {
    this.erc20Services = erc20Services2
    this.ethers   = erc20Services2.ethers
    this.provider = erc20Services2.provider
    this.chainId = erc20Services2.chainId
    UTS = new UniTokenServices(erc20Services2.ethers, erc20Services2.chainId, erc20Services2.provider)
    erc20Services = new ERC20Services(erc20Services2.ethers, erc20Services2.provider)
    // erc20Services = erc20Services2
    router = new AlphaRouter({ chainId: erc20Services2.chainId, provider: erc20Services2.provider})
    console.log( "DEBUG MODE OFF: AlphaRouterService()");
  }

  getRoute = async(_tradeType, _recipientAddr, _tokenAddrIn, _tokenAddrOut, _amount, _slippagePercent) => {
    // console.log( "==========================================================================================================" );
    // console.log( " EXECUTING getRoute(", _tradeType, _recipientAddr, _tokenAddrIn, _tokenAddrOut, _amount, _slippagePercent, ")" );
  ...
  exeExactInputTransaction = async(
    ...
    _gasLimit) => {
      ...
      let route = await this.getRoute( tradeType, _walletAddress, _tokenInAddr, _tokenOutAddr, _exactInputAmount, _slippagePercent);
  ...
  exeExactOutputTransaction = async(
    ...
    _gasLimit) => {
      ...
      let route = await this.getRoute( tradeType, _walletAddress, _tokenInAddr, _tokenOutAddr, _exactOutputAmount, _slippagePercent);
      let quote = route.quote;
      ...
      const transaction = this.getTransaction(route, _walletAddress, _gasLimit)
      const wallet = new this.ethers.Wallet(_walletPvtKey)
